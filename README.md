...
- 🖼️ **结果预览** - 拼接后预览每个分组的长图，支持单条下载与一键下载全部
- ↩️ **撤销/重做** - 完整的操作历史管理
- 💾 **本地存储** - 图片存入 IndexedDB，状态自动保存，刷新不丢失
...
> 运行时基于原生 DOM + TypeScript（无 Vue/React 等框架依赖），通过状态类与事件总线驱动 UI。
...
4. **设置参数** - 调整输出质量、格式等设置
5. **开始拼接** - 点击"开始拼接"按钮，处理完成后在"拼接结果"区预览每个分组的合成图
6. **下载结果** - 点击单个"下载"或"下载全部"保存长图
...
- 🐛 修复撤销/重做、分组操作（建组/删组/清空/拖图进组）不持久化的问题，刷新后不再丢失
- 🖼️ 新增拼接结果预览与单条/批量下载（文件名按"分组名_时间戳"命名）
- 💾 图片数据改存 IndexedDB，localStorage 仅存轻量元数据，规避 5MB 配额限制
- 🧹 清理 Vue 脚手架残留，运行时为纯 TypeScript + 原生 DOM
