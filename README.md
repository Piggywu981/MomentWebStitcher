...
📊 **实时预览**：处理进度实时显示  
🧹 **一键清理**：内置清理功能  
⏰ **时间排序**：按拍摄时间自动排序（早到晚；无拍摄时间的图片按文件时间排在其后）  
...
- **分组大小**：自定义每组图片数量
- **输出宽度**：拼接结果宽度自动限制在 1080px 以内（超长图会等比缩小），适配朋友圈显示
...
#### 🎨 拖拽操作
- 支持图片在分组间自由拖拽（按落点位置插入）
- 支持从图片池拖拽到分组
- 支持清空分组后图片回到图片池
- **组内排序**：支持在分组内拖拽调整图片顺序（鼠标与触屏均支持）
...
### 自定义配置
在 `script.js` 顶部可调整以下常量：
```javascript
// 输出限制：宽度 1080px 对朋友圈足够（微信会二次压缩），
// 总面积上限规避移动端 Safari 大 canvas 输出空白的问题
const MAX_OUTPUT_WIDTH = 1080;        // 拼接结果最大宽度（px）
const MAX_CANVAS_AREA = 16777216;     // canvas 最大面积（px²）
```
...
运行测试（Playwright 无头浏览器冒烟测试，覆盖上传/EXIF/拖拽/触摸/拼接主流程）：
```bash
npm i -D playwright
npx playwright install chromium
npm test
```
测试是可选的开发依赖，不影响线上零依赖的运行方式。
...
**Q: 为什么有些照片上传后不见了？**
A: 浏览器无法解码的格式（如 iPhone 的 HEIC 原图）会被自动跳过并提示，可先转换为 JPG/PNG 再上传
